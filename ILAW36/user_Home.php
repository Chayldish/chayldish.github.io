...
<!-- Modal Structure -->
<div class="modal fade" id="add-resident-modal" tabindex="-1" role="dialog" aria-labelledby="addResidentModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            
        <form action="admin_Actions.php" method="post" enctype="multipart/form-data">
    <div class="modal-header">
        <h5 class="modal-title" id="addResidentModalLabel">Upload Image</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
        </button>
    </div>
    <div class="modal-body">
        <!-- Hidden input to pass resident_id -->
        <input type="hidden" name="resident_id" value="<?php echo $_SESSION['session_login']['resident_id']; ?>">
        <input type="hidden" name="last_name" value="<?php echo $_SESSION['session_login']['last_name']; ?>">
        <input type="hidden" name="middle_name" value="<?php echo $_SESSION['session_login']['middle_name']; ?>">
        <input type="hidden" name="first_name" value="<?php echo $_SESSION['session_login']['first_name']; ?>">
        <input type="hidden" name="submission_date" value="<?php echo $formatted_submission_date; ?>">

        <!-- File input form group for image upload -->
        <div class="col-md-12 form-group text-center">
            <input type="file" name="myfiles" accept="image/*" id="input_file" required>
        </div>
    </div>
    <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-dismiss="modal">Cancel</button>
        <button type="submit" class="btn btn-primary" id="btn_add_photo" name="btn_add_photo">UPLOAD</button>
    </div>
</form>
        </div>
    </div>
